...
class vunidadMedidaController extends Controller{
    ...
    public function __construct() {        
        $this->loadModel(array('modulo'=>'ventas','modelo'=>'vunidadMedida'));
    }
    ...
    public function getGridVunidadMedida(){
        ...
        $eliminar = Session::getPermiso("VUNIDDE");
        ...
        if(!isset($rResult['error'])){  
            ...
            foreach ( $rResult as $key=>$aRow ){
                ...
                if($eliminar['permiso']){
                     if($aRow['uso'] == 0):
                        $axion .= '<button type=\"button\" class=\"'.$eliminar['theme'].'\" title=\"'.$eliminar['accion'].'\" onclick=\"vunidadMedida.postDeleteVunidadMedida(this,\''.$encryptReg.'\')\">';
    ...
    public function postDeleteVunidadMedida(){
        $data = Obj::run()->vunidadMedidaModel->deleteVunidadMedida();
